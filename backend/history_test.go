...
func TestPostgresStore_RoundTrip(t *testing.T) {
	...
	in, _ := json.Marshal(Input{Worker: "Radha", Lines: []Line{{Category: "stitch", Qty: 120, Rejects: 5, Rate: 3}}})
	saved, err := store.Save(Record{Input: in, Headline: 345, Label: "Radha"})
	if err != nil {
		t.Fatalf("save: %v", err)
	}
	if saved.ID == 0 {
	...
	if len(items) == 0 || items[0].ID != saved.ID || items[0].Label != "Radha" {
